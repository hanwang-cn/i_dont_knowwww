...
t=np.linspace(0,110,111,endpoint=True)
y_moi=[]
moi=np.linspace(10,90,150,endpoint=True)

for m in moi:
    y_a=solve_ivp(deriv,t_span=(0,120),y0=y_0,method='DOP853', t_eval=t,args=(30,m),rtol=3e-14,atol=3e-18)
    y_moi.append(y_a.y[6][-1])

fig=plt.figure(dpi=120,tight_layout=True)

def showplt():
    plt.scatter(moi,y_moi,color='r',marker='*',label='Data Point')
    plt.show()


def writexlsx():
    data = {'moisture':moi,'rate': y_moi}
    df = DataFrame(data)
    df.to_excel('moi_decompos1.xlsx')
