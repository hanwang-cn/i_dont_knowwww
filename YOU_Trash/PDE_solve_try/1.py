from pde import CartesianGrid, ScalarField, MemoryStorage, PDE, plot_kymograph
import numpy as np
...
tmin=0
tmax=500
bc_T=f'{h}/{llambda}*(Piecewise(((25+{T_Kelvin}),tt<=20/{v}),\
...
eq = PDE(rhs={'c': f'{alpha}*laplace(c)'}, 
    bc={'type': 'mixed', 'value': -1*h/llambda, 'const': bc_T},)
    #bc_ops={})
